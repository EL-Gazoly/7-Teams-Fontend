...
const TotalGradeForStage = ({ experiments }) => {
  ...
  const calculateTotal = () => {
    let theorticalMax = [];
    let practicalMax = [];
    experiments.teamByName.forEach((team) => {
  ...
  const data = {
    ...
    datasets: [
      {
        label: getTranslation("students_count"),
        data: [
    ...
    <div className='w-[457px] h-[354px] py-[31px] px-7 bg-white dark:bg-primary-dark text-[#444] dark:text-white rounded-lg flex flex-col gap-y-5 relative'>
      <div className="flex w-full items-center justify-between">
        <span className='text-sm font-bold'>
          {`${getTranslation('overall_estimate')} ${
        ...
        <div className='flex items-center gap-x-2'>
          <div className='flex items-center gap-x-1'>
            ...
            <div className='w-[69px] text-[8px] font-semibold'>
              {getTranslation('detailed_estimate_practical')}
            </div>
          </div>
          <div className='flex items-center gap-x-1'>
            <div className='w-1 h-8 bg-[#05C283] rounded' />
            <div className='w-[69px] text-[8px] font-semibold'>
              {getTranslation('detailed_estimate_theoretical')}
            </div>
          </div>
        </div>
      </div>
