...
const ChooseClassAlpha = (props) => {
  ...
  useEffect(() => {
    ...
    } else if (props.selectedClassAlpha) {
      setSelectedOption(options.find((option) => option.value === props.selectedClassAlpha));
    }
  ...
  return (
    <div className="App">
      <Select
        options={options}
        styles={customStyles}
        placeholder={getTranslation('chooseClass')}
        value={selectedOption}
        onChange={handleChange}
        isSearchable={true}
        //@ts-ignore
        getOptionLabel={(option) => (
          <div className='selected flex items-center gap-x-6 text-xs font-medium' style={{ direction: language=== "ar" ? "rtl" : "ltr" }}>
