...
import { createStudent, getStudent } from '../../graphql/students';
...
const UpdateStudent = () => {
  ...
  const { data: studentData, loading: studentLoading, error: StudentError } = useQuery(getStudent, {
    variables: {
      studentId: id,
    },
  });
  ...
  useEffect(() => {
    if (studentData){
      if (studentData.student.imageUrl) {
        setSelectedImage(`${import.meta.env.VITE_API_URL}${studentData.student.imageUrl}`);
    }
    setSelectedSchool({
    ...
    setSelectedSchool(studentData.student.team.school);
