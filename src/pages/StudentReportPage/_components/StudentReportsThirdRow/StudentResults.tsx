...
const StudentResults = ({ data }) => {
  ...
  return (
    <div className='w-[458px] h-[354px] px-9 py-7 bg-white dark:bg-primary-dark rounded-lg flex flex-col gap-y-5'>
      <span className='text-text-black dark:text-white text-sm font-bold'>{getTranslation('practical_and_theoretical_results')}</span>
      <div className='flex flex-col gap-y-4'>
        <div className="flex items-center justify-between">
          <div className='flex items-center gap-x-[33px] text-[#96A5B8] dark:text-[#EAEBEC] text-[10px]'>
            <span>#</span>
            <span>{getTranslation('course_logo')}</span>
            <span>{getTranslation('course_name')}</span>
          </div>
          <span className='text-[#96A5B8] dark:text-[#EAEBEC] text-[10px] ml-6'>{getTranslation('grade')}</span>
        </div>
        ...
        <div className='flex flex-col gap-y-[11px]'>
          <div className='w-full flex items-center justify-end gap-4 text-[#96A5B8] dark:text-[#EAEBEC] text-[10px]'>
            <span>{getTranslation('practical')}</span>
            <span>{getTranslation('theoretical')}</span>
          </div>
          ...
          <div className='w-full flex items-center justify-between'>
            <div className='flex items-center gap-x-[33px] text-text-black dark:text-white text-xs font-medium'>
              ...
              <span>{getTranslation('chemistry')}</span>
            </div>
            <div className={cn('flex items-center ml-2', language === "ar" ? " gap-x-8" : " gap-x-16")} style={{ 
          ...
          <div className='w-full flex items-center justify-between'>
            <div className='flex items-center gap-x-[33px] text-text-black dark:text-white text-xs font-medium'>
              <span>02</span>
              <img src={Physics} alt="" />
              <span>{getTranslation('physics')}</span>
            </div>
            <div className={cn('flex items-center ml-2', language === "ar" ? " gap-x-8" : " gap-x-16")} style={{ direction: language === 'ar' ? 'rtl' : 'ltr' }}>
