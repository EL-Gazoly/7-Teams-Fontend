...
const ChooseCourseSection = ({ selectedHeadsets, setSelectedHeadsets }: ChooseCourseSectionProps) => {
  ...
  return (
    <div
      className='w-full bg-white/50 dark:bg-[#262B34]/50 h-[85px] inline-flex items-center px-[30px] gap-x-4'
      style={{ direction: language === "ar" ? "rtl" : "ltr"}}
      key={key}
    >
      <SelectSubject
        options={options}
        SelectdSubject={SelectdSubject}
        setSelectedSubject={setSelectedSubject}
        setSelectedChapter={setSelectedChapter}
        setSelectedExpriemnt={setSelectedExpriemnt}
      />
      <SelectChapter
        options={options}
        SelectdSubject={SelectdSubject}
      ...
      <SelectExpirment
        ...
        SelectdSubject={SelectdSubject}
        SelectedChapter={SelectedChapter}
        SelectedExpirment={SelectedExpirment}
        setSelectedExpriemnt={setSelectedExpriemnt}
      />
      <SelectHowToStart
        SelectHowToStart={SelectedHowToStart}
        setSelectedHowToStart={setSelectedHowToStart}
      />

      <Button
        className='flex items-center justify-center px-[14px] py-[7px] rounded-[7px]'
        color='primary'
        onPress={handelOpenCourse}
        isDisabled={!(SelectdSubject && SelectedChapter && SelectedExpirment && SelectedHowToStart)}
      >
        {getTranslation("start_experiment")}
      </Button>
    </div>
  );
};
