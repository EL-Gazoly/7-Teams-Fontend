      ...
      >
        <div className="relative w-full h-[37px] pt-3">
          <div className="flex items-center gap-x-1">
            ...
            {device?.StudentPing && (
              <Tooltip
                content={getTranslation("student_help")}
                placement="top"
                className="bg-[#EDB200] text-white"
                offset={4}
              >
                <div
                  className="w-[26px] h-6 bg-[#EDB200] rounded-lg flex items-center justify-center"
                  onClick={updateStudentPing}
                >
          ...
          >
            <span className="text-[9.7px] font-semibold text-white">
              {device.student.length === 0
                ? getTranslation("not_connected")
                : getTranslation("connected")}
            </span>
            <div
              className={`w-[6.979px] h-[6.979px] rounded-full ${
                device.student.length === 0 ? "bg-[#DFEBE1]" : "bg-[#45FF5F]"
              }`}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-y-2 text-center overflow-hidden h-[182px] mt-3">
          ...
          <div className="flex items-center gap-x-[3px]">
            {[0, 33, 66].map((level) => (
              <div
                key={level}
                className={`w-5 h-2 rounded-sm ${batteryLevelClass(
                  device?.Battery
                )}`}
              />
            ))}
          ...
          <div className="flex flex-col gap-y-2">
            <span className="text-[#122333] dark:text-white text-sm font-semibold">
              {device.student.length > 0
                ? device.student[0].name
                : getTranslation("noone")}
            </span>
            <span className="text-[#A5A5A5] text-[11px]">
              #
              {device.student.length > 0
                ? device.student[0]?.facilityId
                : "0000"}
            </span>
          </div>
        </div>
