...
export function FirstCard({ device, deviceState }) {
  ...
  return (
    ...
    `}
      ...
      <div className={cn(`absolute top-0  w-[116px] h-9 py-2 px-[6px] flex items-center justify-center gap-x-[6px] rounded-b-[3px]
      ...
      bg-primary`,
        ...
        <span className='text-white'>
          {device.student.length !== 0 ? getTranslation('connected') : getTranslation('disconnected')}
        </span>
      </div>
        ...
        {show &&
          ...
          >
            ...
            <span className='text-sm font-semibold'>
              {isClicked ? getTranslation('completed') :
                isHovered ? getTranslation('repair') : getTranslation('help')
              }
            </span>
          </div>
        }
      ...
      <div className='flex flex-col items-center justify-center w-full gap-y-2'>
        ...
        <div className="flex flex-col gap-y-1 items-center text-text-black dark:text-white text-center">
          <span className='text-[22px] font-bold'>{device.student.length !== 0 ? device.student[0].name : getTranslation('noStudent')}</span>
