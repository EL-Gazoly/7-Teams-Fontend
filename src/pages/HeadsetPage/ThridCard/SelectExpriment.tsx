...
const SelectExpriment = ({
  ...
  return (
    <Dropdown
      classNames={{
        content:
          "bg-[#444444] dark:bg-[#444850] text-white font-bold text-xs rounded",
      }}
      className="z-10"
    >
      <DropdownTrigger>
        <Button
          className="w-[154px] p-2 gap-x-3 bg-[#444444] dark:bg-[#444850] text-white font-bold text-xs rounded-md"
      ...
      <DropdownMenu aria-label="select-experiment">
        {options.length !== 0 &&
          options.map((option, index) => (
            <DropdownItem
              key={index}
              className="p-3 flex flex-row-reverse items-center gap-x-3 text-right border-b border-[#FFFEFE29] rounded-none"
              startContent={
                <Image
                  src={option?.icon}
                  width={18}
