...
const SecondCard = ({ rolesCount }) => {
  ...
  useEffect(() => {
    if (rolesCount) {
      ...
      const supervisorRoles = rolesCount.filter((role) => role.name === 'المدير التنفيذي');

      const countUsers = (roles) => roles.reduce((acc, role) => acc + role.users.length, 0);
      ...
      const newTotalCount = newTeacherCount + newAdminCount + newSupervisorCount;
  ...
  const data = {
    labels: [getTranslation('system_admin'), getTranslation('executive_admins'), getTranslation('teachers_supervisors')],
    datasets: [
      {
  ...
  const calculatePercentage = (count) => (totalCount ? Math.floor((count / totalCount) * 100) : 0);

  return (
    <div className="w-[289px] h-[254px] rounded-2xl bg-white dark:bg-[#252A33] dark:text-white py-[10px] flex flex-col gap-y-5 relative">
      <span className="text-base font-bold mx-4">{getTranslation('total_admins')}</span>
          ...
          <div className="flex flex-col gap-y-[14px] text-xs text-[#444] font-semibold text-right">
            <div className="flex flex-col items-center gap-y-2 text-[10px]">
              <div className="flex items-center gap-x-1 w-[118px]">
                ...
                <span className="dark:text-white">{calculatePercentage(adminCount)}% {getTranslation('system_admin')}</span>
              </div>
              <div className="flex items-center gap-x-1 w-[118px]">
                <div className="w-3 h-2 rounded bg-[#21FB45]" />
                <span className="dark:text-white">{calculatePercentage(supervisorCount)}% {getTranslation('executive_admins')}</span>
              </div>
              <div className="flex items-center gap-x-1 w-[118px]">
                <div className={cn("w-3 h-2 rounded bg-[#8DF49D] ",
                  language === 'ar' ? 'mb-4' : "mb-0"
                )} />
                <span className="dark:text-white">{calculatePercentage(teacherCount)}% {getTranslation('teachers_supervisors')}</span>
              </div>
            </div>
          </div>
        </div>
        <div className={cn(`absolute top-[32%]  w-[140px] h-[140px]`,
