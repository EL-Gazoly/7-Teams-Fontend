  ...
  useEffect(() => {
    if (studentByGrade) {
      const countStudents = (students) => {
        let counters = {};
        for (let i = 65; i < 75; i++) {
          // ASCII codes for letters A to J
      ...
      setMiddleSecondCount(
        countStudents(studentByGrade[1].classes[1].students)
      );
      setMiddleThirdCount(countStudents(studentByGrade[1].classes[2].students));
      setPrimaryFirstCount(
        countStudents(studentByGrade[2].classes[0].students)
      );
      setPrimarySecondCount(
        countStudents(studentByGrade[2].classes[1].students)
      );
      setPrimaryThirdCount(
        countStudents(studentByGrade[2].classes[2].students)
      );
      setPrimaryFourthCount(
        countStudents(studentByGrade[2].classes[3].students)
      );
      setPrimaryFifthCount(
        countStudents(studentByGrade[2].classes[4].students)
      );
      setPrimarySixthCount(
        countStudents(studentByGrade[2].classes[5].students)
      );
    }
  }, [studentByGrade]);
  ...
  return (
    <div className=" w-[408px]  h-[254px] ">
      <div className=" w-full h-full bg-white  dark:bg-[#252A33] rounded-2xl p-[10px] flex flex-col  gap-y-5">
        <span className=" text-[#444] dark:text-white font-bold tracking-[-0.01119rem]">
          {getTranslation("studentCountInEachStage")}
        </span>
