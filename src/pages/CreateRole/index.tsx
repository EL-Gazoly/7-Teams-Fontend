...
const CreateRolePage = () => {
  ...
  if (data) {
    toast.success(getTranslation('roleAddedSuccessfully'));
  }

  return (
    <>
      <ControlCard icon="Settings" title={getTranslation('roles')} neasted={true} info='createRolePageDescription' />
      <div className='mt-[38px] flex flex-col items-center gap-y-7 pb-9'>
        <div className='flex items-center justify-center w-full'>
          ...
          >
            ...
            <span>{getTranslation('addNewRole')}</span>
          </Button>
        </div>
        <RolesSection 
          disabled={disabled} 
        ...
        <div className='w-full items-center justify-center flex mt-9'>
         {!disabled && <Button color='primary' className='w-32 h-12 py-[6px] px-4 rounded-md' onPress={handleSave}>
            {getTranslation('save')}
          </Button>}
        </div>
      </div>
